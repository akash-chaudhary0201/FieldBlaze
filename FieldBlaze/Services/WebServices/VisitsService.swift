...
class VisitsService{
    ...
    //Function to get  visits based on account:
    public static func getVisitByAccount(_ accountId:String) async{
        let soqlQuery = """
                SELECT Id, Name, Account_RE__r.Name, Visit_Approval_Status__c, CreatedDate, Zone_RE__r.Name FROM Visit__c where Account_RE__r.Id = '\(accountId)' ORDER BY CreatedDate DESC
            """
        guard let encodedQuery = soqlQuery.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed),
              let instanceUrl = Defaults.instanceUrl else {
            return
        ...
        do{
            ...
            GlobalData.customerVisits.removeAll()
            ...
            if let jsonData = try JSONSerialization.jsonObject(with: data, options: []) as? [String:Any],
               let records = jsonData["records"] as? [[String:Any]]{
                for record in records {
                    GlobalData.customerVisits.append(VisitsModel(dict: record))
                }
            }
        }catch{
            print("Error: \(error)")
